...
STAGE = "prepare_batch_docking.optimize"
...
if __name__ == '__main__':
    ...
    prd_res_fn = use_path(file_path=saved_dn / f"optimize/prd_res{opt.suffix}.json")
    ...
    log_fn = [use_path(file_path=saved_dn / f"{STAGE}.log"),
              use_path(file_path=saved_dn / f"optimize/batch_docking_input{opt.new_suffix}.log")]
    tarball_fn = use_path(file_path=saved_dn / f"optimize/batch_docking_input{opt.new_suffix}.auto")
    lookup_fn = use_path(file_path=saved_dn / f"optimize/batch_docking_lookup{opt.new_suffix}.json")
    ...
    # Lookup Table
    prd_res_dic = auto_load(prd_res_fn)
    lookup_dic = dict()
    ...
    pbar = tqdm(prd_res_dic.items(), desc='Batch Prepare (add)')
    for cid, result in pbar:
        result = edict(result)
        meta = id_to_test_meta[cid.split(';')[0]]
        lookup = edict()

        complex = bp.add_complex(
            receptor=(bp.ReceptorTypeEnum.PDB_FN, sbdd_dn / meta.protein),
            ligand=(bp.LigandTypeEnum.SMI_STR, result.ref_smi),
            box=(bp.BoxTypeEnum.SDF_FN, sbdd_dn / meta.ligand),
        )
        lookup.ref = complex.id
        complex = bp.add_complex(
            receptor=(bp.ReceptorTypeEnum.PDB_FN, sbdd_dn / meta.protein),
            ligand=(bp.LigandTypeEnum.SMI_STR, result.prd_smi),
            box=(bp.BoxTypeEnum.SDF_FN, sbdd_dn / meta.ligand),
        )
        lookup.prd = complex.id

        lookup_dic[cid] = lookup
        pbar.set_postfix({
