...
def parse_opt():
    parser = argparse.ArgumentParser(description='Evaluate for optimize: QED, SA, Lipinski, LogP - CVAE Complex')
...
if __name__ == '__main__':
    ...
    log_fn = [use_path(file_path=saved_dn / f"{STAGE}.log"),
              use_path(file_path=saved_dn / f"optimize/qedsa{opt.suffix}.log")]
    qedsa_fn = use_path(file_path=saved_dn / f"optimize/qedsa{opt.suffix}.json")
    ...
    smi_set = set(dic[f'{mod}_smi'] for dic in prd_res_dic.values() for mod in ('ref', 'prd'))
